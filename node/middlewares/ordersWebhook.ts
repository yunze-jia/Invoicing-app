...
export async function ordersWebhook(ctx: any) {
    ...
    try {
        ...
        const sellerEmail: any = await getSellerEmailById(orderDetails.sellers, account)
        const date = new Date();
        const body: any = {
        ...
        const buyerDetails = await notifyBuyer(payload.OrderId, orderDetails.clientProfileData.email, account, invoiceNumber.DocumentId);
        sellerEmail.forEach(async (data: any) => {
            await
                notifySeller(payload.OrderId, data, account, invoiceNumber.DocumentId);
        })
...
async function notifyBuyer(orderId: any, email: string, account: string, invoiceNo: string) {
    ...
    const options = {
        ...
        data: {
            ...
            'jsonData': {
                'email': email,
                'invoiceUrl': `https://tnia--vtexasia.myvtex.com/invoice/buyer/${orderId}/${invoiceNo}`,
...
async function notifySeller(orderId: any, data: any, account: string, invoiceNo: string) {
    console.log('the order id is : ' + orderId + ' the account is : ' + account)
    const options = {
        method: 'POST',
        url: `https://${account}.${constants.VTEX_COMMERCE_BASE_URL}/mail-service/pvt/sendmail`,
        ...
        data: {
            ...
            'jsonData': {
                'email': data.email,
                'invoiceUrl': `https://tnia--vtexasia.myvtex.com/invoice/seller/${data.sellerId}/${orderId}/${invoiceNo}`,
                'message': 'This is a test'
            }
        }
    };
...
async function getSellerEmailById(sellers: any, account: string) {
    const ids = sellers.map((o: any) => o.id)
